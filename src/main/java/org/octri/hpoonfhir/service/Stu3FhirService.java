...
public class Stu3FhirService extends AbstractFhirService {
	...
	public Stu3FhirService(String serviceName) {
		super(serviceName);
	}
	...
	public Patient findPatientById(Session session, String id) throws FHIRException {
		return session.read().resource(Patient.class).withId(id).execute();
	}
	...
	public List<Patient> findPatientsByFullName(Session session, String firstName, String lastName) throws FHIRException {
		...
		Bundle patientBundle = session.search().forResource(Patient.class).where(Patient.FAMILY.matches().value(lastName)).and(Patient.GIVEN.matches().value(firstName)).returnBundle(Bundle.class).execute();
	...
	public List<Observation> findObservationsForPatient(Session session, String patientId) throws FHIRException {
		...
		Bundle observationBundle = session.search().forResource(Observation.class).where(new ReferenceClientParam("patient").hasId(patientId)).returnBundle(Bundle.class).execute();
